    ...
    def add_command(self, command: str):
        ...
        try:
            ...
            terminal_processor.update_terminal_history([
                {
                    "type": "command",
                    "content": terminal_processor.clean_command(command),
                    "timestamp": datetime.now().strftime("%H:%M:%S"),
                }
            ])
            if self.placeholder:
                self.render_terminal_display(terminal_processor.get_terminal_history())
        except Exception:
            if self.placeholder:
                self.render_terminal_display([
                    {
                        "type": "command",
                        "content": command,
                        "timestamp": datetime.now().strftime("%H:%M:%S"),
    ...
    def add_output(self, output: str):
        ...
        try:
            ...
            terminal_processor.update_terminal_history([
                {
                    "type": "output",
                    "content": terminal_processor.sanitize_output(output),
                    "timestamp": datetime.now().strftime("%H:%M:%S"),
                }
            ])
            if self.placeholder:
