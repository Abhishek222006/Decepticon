...
from frontend.web.utils.constants import COMPANY_LINK
...
class ThemeUIComponent:
    ...
    def render_corner_logo(self, link_url: Optional[str] = None, size_px: int = 34):
        """좌측 상단 고정 로고 렌더링 (랜딩 페이지 링크)

        Args:
            link_url: 클릭 시 이동할 URL (기본값: COMPANY_LINK)
            size_px: 로고 크기(px)
        """
        url = link_url or COMPANY_LINK

        logo_path = self.base_path / "assets" / "decepticon_logo.png"
        if not logo_path.exists():
            st.markdown(f"[Decepticon]({url})")
            return
        ...
        st.markdown(
            ...
            <style>
              ...
              a.decepticon-corner-logo:hover {{
                background: rgba(0, 0, 0, 0.35);
              }}
            </style>

            <a class="decepticon-corner-logo" href="{url}" target="_blank" rel="noopener noreferrer">
              <img src="data:image/png;base64,{encoded}" alt="Decepticon" />
            </a>
            """,
            unsafe_allow_html=True,
        )

    def render_sidebar_logo(self, link_url: Optional[str] = None, size_px: int = 34):
        """사이드바 상단 로고 렌더링 (랜딩 페이지 링크)
        ...
        st.markdown(
            ...
            <style>
              ...
              a.decepticon-sidebar-logo img {{
                width: {size_px}px;
                height: {size_px}px;
                border-radius: 8px;
                display: block;
              }}
              a.decepticon-sidebar-logo:hover {{
                background: rgba(0, 0, 0, 0.30);
              }}
            </style>

            <div style="margin-bottom: 8px;">
              <a class="decepticon-sidebar-logo" href="{url}" target="_blank" rel="noopener noreferrer">
                <img src="data:image/png;base64,{encoded}" alt="Decepticon" />
              </a>
            </div>
            """,
